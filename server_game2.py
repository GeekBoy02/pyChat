...
def handle_client(client_socket, client_address):
    ...
    if username not in user_data:
        user_data[username] = {"messages_sent": 0, "times_connected": 1, "class": "Soldier",
                               "level": 1, "xp": 0, "health": 100, "speed": 15, "intellect": 7, "luck": 10, "freeAP": 1, "inventory": inventory}
        save_user_data()
...
class Player:
    ...
    def attack_enemy(self, enemy):
        ...
        if enemy.health <= 0:
            ...
            modify_inventory('user_data.json', self.name, 'Credits', 33)
            xp = enemy.level * 10
            self.xp += xp
            user_data[self.name]["xp"] = self.xp
            save_user_data()
...
def modify_inventory(user_data, username, item_name, quantity_change):
    # Load the user_data JSON file into a Python dictionary
    with open(user_data, 'r') as f:
        data = json.load(f)

    # Check if the given user exists in the data
    if username in data:
        # Check if the given item exists in the user's inventory
        if item_name in data[username]['inventory']:
            # If so, update the quantity by adding or subtracting the change
            data[username]['inventory'][item_name] += quantity_change
            if data[username]['inventory'][item_name] < 0:
                # If the quantity goes negative, set it to zero
                data[username]['inventory'][item_name] = 0
        else:
            # If the item doesn't exist in the user's inventory, add it with the given quantity
            data[username]['inventory'][item_name] = max(quantity_change, 0)
    else:
        # If the user doesn't exist in the data, print an error message
        print(f'Error: user "{username}" not found.')

    # Write the modified data back to the user_data file
